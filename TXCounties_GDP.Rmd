...
gdp_leaf1 <- spread(gdp_leaf, industry, gdp)
...
# adding data to fill
palette <- colorBin("Blues", bins = 5, domain = df$gdp, na.color = "#f0f9e8")

leaflet() %>% 
  addProviderTiles("CartoDB.Positron") %>% 
  setView(-99.9, 31.97, zoom = 7) %>% 
  addPolygons(
    data = df,
    color = 'white',
    weight = 1,
    fillColor = palette(df$gdp),
    fillOpacity = 0.7,
    popup = paste0(df$county, "</br>", "GDP:", df$gdp)) %>% 
  addMarkers(data = )
```
